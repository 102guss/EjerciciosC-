...
int npar(int *vectoor1, int taam)/*recibimos la direccion de memoria de un vector que se le está pasando
...

{
	...
	{	
		...
		apuntando a un apuntador de apuntadores, osea, a una matriz*/
          par++;                    //contador que indicará la cantidad de numeros pares que abrá en el vector
	}
...
int *fun(int *vectooor1, int taaam){  //aqui si me va aretornar un apuntador  a un vector de enteros  //estoy trabajando con mi vector original atraves de *vectooor1
	...
	nuevovector= (int *)malloc(   npar(vectooor1,taaam)   *sizeof(int));//apartame el tamaño necesario qu ocupa npar(funcion anterior) y asignaselo a nueovector
	...
	for(i=0; i<taaam; i++){
		...
		}                                                                //de esa manera el nuevo vector se quedará con los elementos necesarios sin desperdiciar espacio
